...
> 目标：以溪东社区为第一阶段，按仓库现有模拟数据规模完成可审核、可试运行、可运维的生产版本
...
本项目当前没有真实运行基线，也没有项目方批准的 KPI 目标。此前版本中的激活率、阅读率、社区减负比例、误扰率和满意度百分比均不是仓库数据，现全部删除。立项阶段只确认指标定义；目标值必须在真实导入清单确认、服务号能力审核和基线期完成后，由项目方书面批准。
...
| 社区工作量 | 每日电话、走访、邻里任务数量及工作人员耗时 | 无试点前基线，先测量后定目标 |
| 误扰 | 经人工确认属于外出、住院、设备问题等已知正常情况的提醒数 / 已处理提醒数 | 无真实数据 |
| 老人体验 | 统一访谈/问卷中关于麻烦程度、监控感和内容价值的回答分布 | 问卷尚未批准，不设百分比 |
| 系统质量 | 请求成功/失败、消息送达、任务积压、越权与数据泄露事件 | 技术测试与生产监控分别记录 |
...
2. 用户研究基线：访谈对象和数量由社区根据正式名单确定，记录当前联系与排查流程；未完成访谈前不写满意度或减负结论。
...
- **G3 允许溪东社区全量运行：** 先完成项目方批准的小范围联调，且未发生重大隐私事件；联调样本数和观察周期由项目方书面确认，不在本文虚构。
- **G4 决定长期运行：** 完成项目方批准周期的指标与访谈复盘，明确持续预算、运维主体和年度目标。
...
- 在项目方批准的体验名单中完成服务号关注、UnionID 绑定、消息送达、点击进入小程序和已读回传全链路测试；体验人数以审批名单为准。
...
- 对话历史首期不长期保存；请求 ID、工作者 ID、工具名、参数摘要、结果条数、模型版本、token 和错误码的审计保存期限由政府项目数据制度确认。日志不记录完整手机号和健康备注。
...
供应商选型不以单次 Demo 效果决定。采购前由项目方批准测试素材清单，对候选 API 比较人物一致性、失真率、违规拦截、时延、成功/失败结果、单次成本、数据保存条款、删除机制和企业合同/SLA。测试素材不得使用未经授权的居民生产数据，素材数量不在本文件中虚构。
...
- 定期执行完整备份校验；备份频率和保存期限由项目方数据制度、RPO/RTO 要求及 OSS 预算共同确认，未批准前不写固定保留天数。
...
验收：按项目方批准的体验名单完成服务号消息送达、点击、小程序打开和已读回传；所有家庭/社区权限测试通过，无跨家庭/跨社区数据泄露。
...
验收：使用项目方批准的文案/解析测试集记录逐条结果，不预设无依据的成功率；Agent 权限测试全部通过；供应商不可用时核心业务不受影响。

### 第 4 阶段：政府 App 联调，预计 1-2 周，可与第 3 阶段并行

- 双方确认数据目录、主数据责任、OpenAPI 合同、鉴权、白名单和错误码。
- 完成概览、老人状态、事件拉取与处置回写；如需要，再开启 webhook。
- 使用虚构数据执行契约、安全、幂等和断网恢复测试；同步容量按 2,101 位老人及关联数据口径验证。

验收：政府 App 正确展示更新时间、信号来源和状态说明；双方可按 request ID 对账；越权 scope、重放请求和未授权字段均被拒绝。

### 第 5 阶段：试运行与审核，预计 2 周

- 首次生产导入按溪东社区正式清单执行，容量上限以仓库基线 2,101 位老人为设计口径；服务号关注和真实家属绑定数量以实际办理结果记录。
- 完成培训、值班表、故障预案、用户反馈和小程序提审。

验收：项目方批准的观察期内无未关闭的 P0 故障；告警闭环可追溯；备份、恢复、注销和数据删除均演练通过。观察期由立项排期确认。
